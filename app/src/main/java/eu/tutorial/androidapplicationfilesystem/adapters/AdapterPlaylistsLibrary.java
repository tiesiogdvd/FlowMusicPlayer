...
public class AdapterPlaylistsLibrary extends RecyclerView.Adapter <AdapterPlaylistsLibrary.ViewHolder> {
    ...
    public void filter(String text){
        musicPlaylistsCopy = null;
        musicPlaylistsCopy = new ArrayList<>();
        if(text.isEmpty()){
            musicPlaylistsCopy = musicPlaylists;
        } else {
            text = text.toLowerCase();
            for(Playlist playlist: musicPlaylists){
                if(playlist.getPlaylistName().contains(text)){
                    musicPlaylistsCopy.add(playlist);
                }
            }
        }
    ...
    public int getItemCount() {
        System.out.println(musicPlaylists.size());
